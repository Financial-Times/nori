...
exports.handler = async (_, state) => Promise.all(state.repos.map(async repo => {
	...
	if (await fs.exists(cloneDirectory)) {
		await git.checkoutBranch({ name: 'master', workingDirectory: cloneDirectory });
	} else {
		await git.clone({ origin: 'origin', repo: remoteUrl, workingDirectory: cloneDirectory });
	}
